...
> yazıldı. Kaynak: `https://linear.app` analizi.
...
Kaynak sitenin yapay zeka izi skoru: **86.4/100** (Özgün ve tutarlı; belirgin şablon izi yok.). Ürettiğin sayfa bu skordan **daha iyi** olmalı.
...
12. **Sayfanın tamamını ortalama.**  
  ...
  *Bu sitede ölçülen:* **%0** (sağlıklı eşik: %30). Bu bandı aşma.
13. **Rastgele boşluk değerleri kullanma.**  
  ...
  *Bu sitede ölçülen:* **%84** (sağlıklı eşik: %88). Bu bandı aşma.
14. **Onlarca farklı punto kullanma.**  
  ...
  *Bu sitede ölçülen:* **8 adet** (sağlıklı eşik: 9 adet). Bu bandı aşma.
15. **WCAG AA eşiğinin altında metin/zemin çifti bırakma.**  
  ...
  *Bu sitede ölçülen:* **%52** (sağlıklı eşik: %2). Bu bandı aşma.
...
- Köşe yarıçapı bu sitede ölçülen değerlerden seçilecek: `5px`, `8px`, `6px`, `2px`, `4px`
- Dolgu düz renk: `#5e6ad2`
- Üstündeki metin: `#ffffff` (kontrast hesabıyla seçildi)
- Parlama/hale/nabız **yok**; hover'da yalnızca ölçülü bir renk veya
  kenarlık değişimi, `0.1s, 0.25s, 0.16s, 0.4s, 0.12s, 0.2s` bandında.
...
2. **Animasyonu zamanlayıcıya bağlama.**  
  ...
  *Ölçüm:* Bu sitede ölçülen: **0** sahne kaydırmaya bağlı, **2** sahne bir kez tetikleniyor.
...
6. **Harf harf açılan başlığı erişilebilirliği bozarak yapma.**  
  ...
  *Ölçüm:* Kaynak sitede ölçülen: **88** harf, **313** kelime sarmalayıcı.
...
- **Renk rolleri:** zemin `#08090a`,
  yüzey `#ffffff`,
  metin `#f7f8f8`,
  ana eylem `#5e6ad2`
- **Metin/zemin kontrastı:** 18.73 (bu değerin altına inme)
- **Tipografi:** `Inter Variable`,
  gövde `16px` /
  ağırlık `400` /
  satır yüksekliği `24px`
