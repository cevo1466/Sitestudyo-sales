...
> yazıldı. Kaynak: `https://attio.com` analizi.
...
Kaynak sitenin yapay zeka izi skoru: **81.2/100** (Genel olarak sağlam; sayılı noktada şablon alışkanlığı var.). Ürettiğin sayfa bu skordan **daha iyi** olmalı.
...
2. **Tam yuvarlak (pill / border-radius:999px) rozet ve etiket enflasyonu YOK.**  
  ...
  *Bu sitede ölçülen:* **0 adet** (sağlıklı eşik: 2 adet). Bu bandı aşma.
3. **Her kutuya birden yuvarlak köşe + gölge verme.**  
  ...
  *Bu sitede ölçülen:* **%2** (sağlıklı eşik: %18). Bu bandı aşma.
...
6. **Cam efekti (backdrop-filter: blur) yüzey enflasyonu YOK.**  
  ...
  *Bu sitede ölçülen:* **2 adet** (sağlıklı eşik: 1 adet). Bu bandı aşma.
7. **Birbirinin tıpatıp aynısı 3'lü kart dizisi YOK.**  
  ...
  *Bu sitede ölçülen:* **4 adet** (sağlıklı eşik: 2 adet). Bu bandı aşma.
...
12. **Sayfanın tamamını ortalama.**  
  ...
  *Bu sitede ölçülen:* **%2** (sağlıklı eşik: %30). Bu bandı aşma.
13. **Rastgele boşluk değerleri kullanma.**  
  ...
  *Bu sitede ölçülen:* **%87** (sağlıklı eşik: %88). Bu bandı aşma.
...
- Köşe yarıçapı bu sitede ölçülen değerlerden seçilecek: `7px`, `30%`, `8px`, `10px`, `12px`
- Dolgu düz renk: `#266df0`
- Üstündeki metin: `#ffffff` (kontrast hesabıyla seçildi)
- Parlama/hale/nabız **yok**; hover'da yalnızca ölçülü bir renk veya
  kenarlık değişimi, `0.3s, 0.4s, 0.15s, 0.2s, 0.5s, 0.6s` bandında.
...
1. **Her elemana aynı `fade-up` animasyonunu verme.**  
  ...
  *Ölçüm:* Kaynak sitede 4 farklı giriş tekniği var, baskın olan `fade` (%57).
2. **Animasyonu zamanlayıcıya bağlama.**  
  ...
  *Ölçüm:* Bu sitede ölçülen: **5** sahne kaydırmaya bağlı, **15** sahne bir kez tetikleniyor.
...
4. **Sabitlenen bölümü 'uzun sayfa' sanıp gereksiz uzatma.**  
  ...
  *Ölçüm:* Bu sitede: **4** sabitlenen bölüm, toplam **4410 px** (~4.9 ekran).
...
6. **Harf harf açılan başlığı erişilebilirliği bozarak yapma.**  
  Bunun yerine: Sarmalayıcıya `aria-label` ver, bölünmüş harflere `aria-hidden="true"` koy.  
  Neden: Metni harflere bölmek ekran okuyucuda başlığı tek tek harf olarak okutur. Efekt kalır, içerik erişilemez hâle gelir.  
  *Ölçüm:* Kaynak sitede ölçülen: **47** harf, **306** kelime sarmalayıcı.
7. **`will-change`'i her elemana verme.**  
  Bunun yerine: Sadece gerçekten animasyonlu elemanlara ver, animasyon bitince kaldır.  
  Neden: Her eleman GPU katmanına alınırsa mobilde bellek şişer ve kaydırma tam da akıcı olması gereken yerde takılır.  
  *Ölçüm:* Kaynak sitede **88** elemanda var — bu sayıyı kopyalama.
...
- **Renk rolleri:** zemin `#e0fced`,
  yüzey `#266df0`,
  metin `#242629`,
  ana eylem `#266df0`
- **Metin/zemin kontrastı:** 14.66 (bu değerin altına inme)
- **Tipografi:** `inter`,
  gövde `16px` /
  ağırlık `500` /
  satır yüksekliği `22px`
